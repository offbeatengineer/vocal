...
#define VOCAL_DEC_MAX_NODES 32768
...
void AudioDecoder::normalize_codebooks() {
...
bool AudioDecoder::load(const std::string & model_path) {
    ...
    if (dec_tensor_count == 0) {
        error_ = "No decoder tensors (tok_dec.*) found in model";
        return false;
    }
    ...
    for (int64_t i = 0; i < n_tensors; ++i) {
        ...
        if (sname == "tok_dec.vq_first.input_proj.weight") model_.vq_first_input_proj = tensor;
            ...
            if (MATCH1("tok_dec.vq_rest.%d.codebook", cb_idx)) {
                if (cb_idx >= 0 && cb_idx < 15) model_.vq_rest_codebook[cb_idx] = tensor;
            }
            else if (MATCH1("tok_dec.vq_rest.%d.usage", cb_idx)) {
                if (cb_idx >= 0 && cb_idx < 15) model_.vq_rest_usage[cb_idx] = tensor;
            }
    ...
    state_.backend = init_preferred_backend("AudioDecoder", &error_);
    ...
    state_.sched = ggml_backend_sched_new(backends.data(), nullptr, (int)backends.size(), VOCAL_DEC_MAX_NODES, false, true);
    if (!state_.sched) {
...
struct ggml_tensor * AudioDecoder::apply_snake(struct ggml_context * ctx,
...
struct ggml_tensor * AudioDecoder::apply_rms_norm(struct ggml_context * ctx,
                                                    struct ggml_tensor * x,
...
struct ggml_tensor * AudioDecoder::apply_pre_tfm_layer(struct ggml_context * ctx,
                                                         struct ggml_tensor * x,
...
struct ggml_tensor * AudioDecoder::apply_upsample_block(struct ggml_context * ctx,
                                                          struct ggml_tensor * x,
    ...
    if (block.pwconv1_b) x = ggml_add(ctx, x, block.pwconv1_b);

    x = ggml_gelu(ctx, x);

    x = ggml_mul_mat(ctx, block.pwconv2_w, x);
    if (block.pwconv2_b) x = ggml_add(ctx, x, block.pwconv2_b);
...
struct ggml_tensor * AudioDecoder::apply_residual_block(struct ggml_context * ctx,
                                                          struct ggml_tensor * x,
...
struct ggml_tensor * AudioDecoder::apply_decoder_block(struct ggml_context * ctx,
                                                         struct ggml_tensor * x,
...
struct ggml_cgraph * AudioDecoder::build_graph(int32_t n_frames) {
    const auto & cfg = model_.config;
    ...
    struct ggml_cgraph * gf = ggml_new_graph_custom(ctx0, VOCAL_DEC_MAX_NODES, false);
    ...
    // VQ lookup: first codebook
    struct ggml_tensor * first_emb = ggml_get_rows(ctx0, model_.vq_first_codebook, cb_codes_tensors[0]);

    // VQ lookup: rest codebooks
    struct ggml_tensor * rest_emb[15];
    for (int cb = 0; cb < 15; ++cb) {
        rest_emb[cb] = ggml_get_rows(ctx0, model_.vq_rest_codebook[cb], cb_codes_tensors[cb + 1]);
    }
    ...
    // Reshape for pre-transformer: [T, C, 1] -> [C, T] -> [T, C] via transpose
    ...
    // Pre-transformer input projection
    ...
    // Pre-transformer output projection
...
std::vector<float> AudioDecoder::decode(const std::vector<std::vector<int32_t>> & codes) {
    ...
    // Transpose: codes[cb][t] -> codes_buf_[t * n_cb + cb]
    codes_buf_.resize(seq_len * cfg.n_codebooks);
    for (int t = 0; t < seq_len; ++t) {
        for (int cb = 0; cb < cfg.n_codebooks; ++cb) {
            int32_t val = 0;
            if (cb < n_codebooks && t < (int)codes[cb].size()) {
                val = codes[cb][t];
            }
            codes_buf_[cb + t * cfg.n_codebooks] = val;
        }
    }

    struct ggml_cgraph * gf = build_graph(seq_len);
    ...
    // Set per-codebook code tensors
    std::vector<int32_t> cb_codes(seq_len);
    for (int cb = 0; cb < 16; ++cb) {
        char name[32];
        snprintf(name, sizeof(name), "codes_cb%d", cb);
        struct ggml_tensor * cb_tensor = ggml_graph_get_tensor(gf, name);
        if (!cb_tensor) {
            error_ = "Failed to find codes tensor for codebook " + std::to_string(cb);
            ggml_backend_sched_reset(state_.sched);
            return {};
        }

        for (int t = 0; t < seq_len; ++t) {
            cb_codes[t] = codes_buf_[t * cfg.n_codebooks + cb];
        }
        ggml_backend_tensor_set(cb_tensor, cb_codes.data(), 0, seq_len * sizeof(int32_t));
    }
    ...
    if (positions_tensor) {
        ...
        ggml_backend_tensor_set(positions_tensor, positions.data(), 0, seq_len * sizeof(int32_t));
    }
    ...
    if (!audio_tensor) {
        error_ = "Failed to find audio tensor";
        ggml_backend_sched_reset(state_.sched);
        return {};
    }
